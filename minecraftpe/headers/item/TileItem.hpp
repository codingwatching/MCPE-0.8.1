...
struct TileItem: Item
{
	...
	virtual bool_t useOn(ItemInstance* item, Player* player, Level* level, int32_t x, int32_t y, int32_t z, int32_t side, float faceX, float faceY, float faceZ) {
		int32_t yNew; // r5
		int32_t zNew; // r6
		int32_t sideNew; // r9
		int32_t blockID; // r2
	...
	virtual bool_t isEmissive(int32_t a2) const {
		int32_t blockID; // r2
		blockID = this->blockID;
