...
struct LiquidTileDynamic: LiquidTile
{
	...
	bool_t canSpreadTo(Level* level, int32_t x, int32_t y, int32_t z) {
		Material* v9; // r0
		v9 = level->getMaterial(x, y, z);
		return v9 != this->material && v9 != Material::lava && !this->isWaterBlocking(level, x, y, z);
	}
	int32_t getHighest(Level* level, int32_t x, int32_t y, int32_t z, int32_t oldhigh) {
		int32_t v7; // r4
		int32_t depth; // r0
		v7 = oldhigh;
	...
	int32_t getSlopeDistance(Level* level, int32_t x, int32_t y, int32_t z, int32_t a6, int32_t a7) {
		int32_t v11; // r4
		int32_t v12; // r11
		int32_t v13; // r5
		int32_t v14; // r6
		...
		do {
			...
			v13 = x;
			LABEL_20: if (!this->isWaterBlocking(level, v13, y, v14) && (this->material != level->getMaterial(v13, y, v14) || level->getData(v13, y, v14))) {
	...
	bool_t* getSpread(Level* level, int32_t x, int32_t y, int32_t z) {
		...
		do {
			...
			LABEL_12: v12 = z;
			LABEL_13: if (!this->isWaterBlocking(level, v11, y, v12) && (this->material != level->getMaterial(v11, y, v12) || level->getData(v11, y, v12))) {
	...
	bool_t isWaterBlocking(Level* level, int32_t x, int32_t y, int32_t z) {
		Tile* v5; // r4
	...
	void setStatic(Level* level, int32_t x, int32_t y, int32_t z) {
		int32_t v9; // r0
	...
	void trySpreadTo(Level* level, int32_t x, int32_t y, int32_t z, int32_t a6) {
		int32_t v10; // r0
		Tile* v11; // r9
		int32_t v13; // r0
	...
	virtual void tick(Level* level, int32_t x, int32_t y, int32_t z, Random* rng) {
		...
		if (depth > 0) {
			...
			if (this->material == Material::lava && depth <= 7) {
				...
				if (v16 > depth) {
					...
					LABEL_32: level->setData(x, y, z, v16, 2);
		...
		LABEL_35: if (this->canSpreadTo(level, x, y - 1, z)) {
