...
struct LiquidTile : Tile{
	...
	LiquidTile(int32_t id, const struct Material* mat, const std::string& a4, const std::string& a5) :
			Tile(id, a4, mat) {
	...
	void fizz(struct Level* level, int32_t x, int32_t y, int32_t z) {
		Random* p_random; // r6
		float v7; // s16
		float v8; // s18
		float f; // s19
	...
	int32_t getDepth(struct Level* level, int32_t x, int32_t y, int32_t z) {
	...
	Vec3 getFlow(LevelSource* level, int32_t x, int32_t y, int32_t z) {
		int32_t v10; // r11
		int32_t v11; // r10
		int32_t v12; // r9
		int32_t v13; // r0
		...
		do {
			...
			v11 = x - 1;
			LABEL_12: v12 = z;
			LABEL_13: v13 = this->getRenderedDepth(level, v11, y, v12);
			if (v13 >= 0) {
				v16 = v13 - depth;
				goto LABEL_18;
			}
	...
	int32_t getRenderedDepth(struct LevelSource* level, int32_t x, int32_t y, int32_t z) {
		int32_t meta; // r0
		if (this->material != level->getMaterial(x, y, z)) {
			return -1;
		}
	...
	static float getSlopeAngle(LevelSource* level, int32_t x, int32_t y, int32_t z, const struct Material* m) {
		float v9; // s15
		float v10; // s14
		if (m == Material::water) {
	...
	void updateLiquid(Level* level, int32_t x, int32_t y, int32_t z) {
		Tile* v9; // r3
		int32_t v10; // r0
		if (level->getTile(x, y, z) == this->blockID && this->material == Material::lava && (Material::water == level->getMaterial(x, y, z - 1) || Material::water == level->getMaterial(x, y, z + 1) || Material::water == level->getMaterial(x - 1, y, z) || Material::water == level->getMaterial(x + 1, y, z) || Material::water == level->getMaterial(x, y + 1, z))) {
			...
			if (v10) {
				if (v10 > 4) {
					LABEL_12: this->fizz(level, x, y, z);
					return;
				}
				v9 = Tile::stoneBrick;
			} else {
	...
	virtual void animateTick(struct Level* level, int32_t x, int32_t y, int32_t z, struct Random* rng) {
		float f1; // s17
		float maxY; // s15
		float f; // r0
	...
	virtual void handleEntityInside(Level* level, int32_t x, int32_t y, int32_t z, Entity* ent, Vec3& a7) {
		float yy; // s13
		float v8; // s15
		float zz; // s13
	...
	const virtual TextureUVCoordinateSet* getTextureNum(int32_t a2) {
