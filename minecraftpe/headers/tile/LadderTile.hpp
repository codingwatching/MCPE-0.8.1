...
struct LadderTile : Tile{
	...
	LadderTile(int32_t id, const std::string& a3, Material* a4) :
			Tile(id, a3, a4) {
	}
	virtual ~LadderTile() {
	}
	...
	virtual AABB getTileAABB(Level* level, int32_t x, int32_t y, int32_t z) {
		...
		this->setShape(0.0, 0.0, 0.875, 1.0, 1.0, 1.0);
		LABEL_10: return Tile::getTileAABB(level, x, y, z);
	}
	...
	virtual void neighborChanged(Level* level, int32_t x, int32_t y, int32_t z, int32_t a6, int32_t a7, int32_t a8, int32_t a9) {
		int32_t v13; // r0
		int32_t v14; // r0
		int32_t v15; // r1
		...
		if (v13 == 2) {
			...
			if (v13 == 4) {
				...
				if (v13 != 5) {
					LABEL_13: this->popResource(level, x, y, z, ItemInstance(Tile::ladder));
		...
		LABEL_9: Tile::neighborChanged(level, x, y, z, a6, a7, a8, a9);
	}
	...
	virtual int32_t getPlacementDataValue(Level* level, int32_t x, int32_t y, int32_t z, int32_t side, float, float, float, Mob*, int32_t a11) {
		int32_t v12; // r4
		v12 = a11;
		if (!a11 || side == 2) {
		...
		LABEL_6: if (!level->isSolidBlockingTile(x, y, z - 1)) {
			LABEL_17: if (!v12) {
				goto LABEL_9;
			}
			goto LABEL_8;
		...
		LABEL_9: if (level->isSolidBlockingTile(x + 1, y, z)) {
			v12 = 4;
			goto LABEL_11;
		}
		if (v12) {
