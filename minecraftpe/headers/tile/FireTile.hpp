...
struct FireTile : Tile{
	...
	FireTile(int32_t ids, const std::string& name) :
	...
	bool_t canBurn(struct LevelSource* level, int32_t x, int32_t y, int32_t z) {
		return this->flammability[level->getTile(x, y, z)] > 0;
	}
	void checkBurn(Level* level, int32_t x, int32_t y, int32_t z, int32_t a6, Random* random) {
		int32_t v11; // r11
		int32_t v12; // r0
		int32_t blockID; // r10
		int32_t v14; // r11
	...
	int32_t getFireOdds(Level* level, int32_t x, int32_t y, int32_t z) {
	...
	int32_t getFlammability(Level* level, int32_t x, int32_t y, int32_t z, int32_t mx) {
		int32_t res = this->flammability[level->getTile(x, y, z)];
		if (res < mx) return mx;

		return res;
	}
	bool_t isValidFireLocation(Level* level, int32_t x, int32_t y, int32_t z) {
	...
	virtual void tick(Level* level, int32_t x, int32_t y, int32_t z, Random* random) {
		...
		if (v11 != blockID) {
			if (!this->isValidFireLocation(level, x, y, z)) {
				...
				STOP_BURNING: level->setTile(x, y, z, 0, 3);
				return;
			}
